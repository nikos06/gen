...
func ObjectId(db *sql.DB, name string) (string, error) {
	return fetchObjectId(db, name)
}
...
func fetchObjectId(db *sql.DB, name string) (string, error) {
	dt := fmt.Sprintf("%T", db.Driver())
	d, ok := driverDialect[dt]
	if !ok {
		return "", UnknownDriverError{Driver: dt}
	}
	// Build and run the appropriate query from dialect:
	// this runs a query that returns no rows, and then
	// picks off the column type info.
	q := fmt.Sprintf(d.queries[objectId], name)
	rows, err := db.Query(q)
	if err != nil {
		return "", err
	}
