...
func (c *Config) GetTemplate(genTemplate *GenTemplate) (*template.Template, error) {
    ...
    var funcMap = template.FuncMap{
        ...
        "firstLetter":       firstLetter,
        "structFieldExists": structFieldExists,
    }
    ...
    if extension == ".go" {
        ...
        if err != nil {
            return nil, fmt.Errorf(
                "error in formatting template: %s outputfile: %s source: %s",
                genTemplate.Name,
                outputFile,
                err.Error(),
            )
...
// ".table.tmpl"
func (c *Config) tableFileHandlerFunc(
    src, dest string,
    info os.FileInfo,
    opt utils.Options,
    results *utils.Results,
